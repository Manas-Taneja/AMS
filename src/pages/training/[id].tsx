...
function TrainingDetail() {
  ...
  const trainingId = parseInt(params?.id || '0')
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="flex items-center gap-4">
          <Image src={training.instructor.image} alt={training.instructor.name} width={64} height={64} className="w-16 h-16 rounded-full object-cover" />
          <div>
            <div className="font-semibold">{training.instructor.name}</div>
            <div className="text-blue-600 text-xs">{training.instructor.credentials}</div>
            <div className="text-gray-600 text-xs">{training.instructor.experience}</div>
          </div>
        </CardContent>
      </Card>
      {/* Schedule Card */}
      <Card>
        ...
        <CardContent>
          <div className="text-sm mb-2"><b>Start:</b> {new Date(training.schedule.startDate).toLocaleDateString()}</div>
          <div className="text-sm mb-2"><b>End:</b> {new Date(training.schedule.endDate).toLocaleDateString()}</div>
          <div className="text-sm mb-2"><b>Format:</b> {training.schedule.format}</div>
          <div className="text-sm"><b>Location:</b> {training.schedule.location}</div>
        </CardContent>
      </Card>
      {/* Stats Card */}
      <Card>
        ...
        <CardContent>
          <div className="text-sm">Enrolled: <b>{training.enrolled}</b></div>
          <div className="text-sm">Completed: <b>{training.completed}</b></div>
          <div className="text-sm">Completion Rate: <b>{completionRate.toFixed(1)}%</b></div>
        </CardContent>
      </Card>
      ...
      {/* Tasks/Issues Placeholder */}
      <Card>
        <CardHeader>
          <CardTitle>Tasks / Issues</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-gray-500 text-sm">No tasks or issues for this training.</div>
        </CardContent>
      </Card>
    </div>
  )
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          <StatsCard icon={<Users className="h-5 w-5 text-blue-500" />} label="Currently Enrolled" value={training.enrolled} bgClass="bg-blue-50" />
          <StatsCard icon={<GraduationCap className="h-5 w-5 text-green-500" />} label="Completed" value={training.completed} bgClass="bg-green-50" />
          <StatsCard icon={<Calendar className="h-5 w-5 text-purple-500" />} label="Start Date" value={new Date(training.schedule.startDate).toLocaleDateString()} bgClass="bg-purple-50" />
        </div>
        {/* Progress Bar */}
        <div className="mt-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-4 mb-8 gap-4">
          ...
          <TabsTrigger value="instructor" className="text-sm font-medium">Instructor</TabsTrigger>
