...
function Projects() {
  ...
  const form = useForm<ProjectFormData>({
    ...
    validationRules: [
      ...
      validationRules.futureDate("dueDate"),
    ],
  ...
  const extendedProjects: ExtendedProject[] = projects.map((project: unknown, index: number) => {
    if (typeof project === 'object' && project !== null) {
      const p = project as Record<string, unknown>;
      return {
        id: String(p.id),
        name: String(p.name),
        description: typeof p.description === 'string' ? p.description : `Project ${p.name} - ${p.status} status with ${p.progress}% progress`,
        ...
        category: String(p.category), // Use backend value as-is
      };
    }
  ...
  const handleModalSubmit = async (data: Partial<ProjectFormData>) => {
    try {
      ...
      if (!data.dueDate) {
        toast.error('Project due date is required');
        return false;
      }
      ...
      const projectData = {
        ...
        owner: data.owner || "IIDT",
        progress: 0, // Default progress for new projects
      };

      if (editMode && editingId) {
        // Show confirmation for update
        const confirmed = await confirmUpdate('project', data.name);
        if (!confirmed) {
          return false;
        }
        const result = await update(editingId, projectData);
        if (result) {
        ...
        const confirmed = await confirmCreate('project');
        if (!confirmed) {
          return false;
        }
  ...
  const projectFields: FieldConfig[] = [
    ...
    { name: "description", label: "Description", type: "textarea", required: true, placeholder: "Enter project description" },
    ...
    { name: "tags", label: "Tags (comma-separated)", type: "text", placeholder: "Enter tags separated by commas" },
  ];
  ...
  const handleExport = () => {
    const csvContent = [
      ['Name', 'Description', 'Status', 'Priority', 'Due Date', 'Progress', 'Owner'],
      ...(projects as ExtendedProject[]).map(project => [
        project.name,
        project.description || '',
        project.status,
        project.priority || '',
          ...
          filters={[
            {
              ...
              onValueChange: (value: string) => setStatusFilter(value as "Active" | "Paused" | "Completed" | "All")
            },
            {
              key: "priority",
              label: "Priority",
              ...
              onValueChange: (value: string) => setPriorityFilter(value as "High" | "Medium" | "Low" | "All")
            }
          ]}
          viewMode={viewMode}
