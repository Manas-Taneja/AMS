...
const Bills: React.FC = () => {
  ...
  const fetchBills = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiService.getWithQuery<Bill[]>('/api/bills', params, token || undefined);
      setBills(data);
  ...
  const fetchStats = useCallback(async () => {
    try {
      const token = sessionStorage.getItem('access_token');
      const data = await apiService.get<BillStats>('/api/bills/stats', token || undefined);
      setStats(data);
    } catch {
      // ignore
    }
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const token = sessionStorage.getItem('access_token');
      const data = await apiService.get<string[]>('/api/bills/categories', token || undefined);
      setCategories(data);
    } catch {
      // ignore
    }
  ...
  const fetchVendors = useCallback(async () => {
    try {
      const token = sessionStorage.getItem('access_token');
      const data = await apiService.get<string[]>('/api/bills/vendors', token || undefined);
      setVendors(data);
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    fetchCategories();
    fetchVendors();
  }, [fetchBills, fetchStats, fetchCategories, fetchVendors]);
      ...
      <div className="p-8 text-black max-w-7xl mx-auto">
        ...
        <ManagerOrAdmin>
          <UploadBillDialog
            ...
            onSuccess={() => { fetchBills(); fetchStats(); }}
